...
----------------------------------------------------------------------------------------------------------------------
function muc.registerCanvasLayoutCategory(mainCategory, subCategorys)
	-- 注册到插件选项界面
	local registerCategory = Settings.RegisterCanvasLayoutCategory(mainCategory.frame, mainCategory.text)
	...
	if #subCategorys > 0 then
		for i, v in ipairs(subCategorys) do
			local subCategory = subCategorys[i]
			local registerSubCategory = Settings.RegisterCanvasLayoutSubcategory(
				registerCategory,
				subCategory.frame,
				subCategory.text)
	...
	return registerCategory, registerSubCategorys
end
...
---创建水平滑动条
----------------------------------------------------------------------------------------------------------------------
function muc.createHorizontalSliderBar(sliderBarName, parentUI, width, height, mdhPotin, soliderHandler)
	-- 创建一个滑动条
	local slider = CreateFrame("Slider", sliderBarName, parentUI, "OptionsSliderTemplate")
	slider:SetSize(width, height)                                                                -- 设置滑动条的宽度和高度
	slider:SetPoint(mdhPotin.point, parentUI, mdhPotin.relativePoint, mdhPotin.offx, mdhPotin.offy) -- 将文本框置于屏幕中央
...
---创建垂直滑动条
----------------------------------------------------------------------------------------------------------------------
function muc.createVerticalSliderBar(sliderBarName, parentUI, width, height, mdhPotin, soliderHandler)
	-- 创建滑动条的滑块
...
---@return FontString
function muc.createProgressBar(progressBarName, parentUI, width, height, barTexture, mdhPotin, minValue, maxValue,
	...
	progressBar:SetPoint(mdhPotin.point, parentUI, mdhPotin.relativePoint, mdhPotin.offx, mdhPotin.offy) -- 将文本框置于屏幕中央
...
function muc.createFont(createframe, mdhPotin, text, fontsize)
	...
	font:SetPoint(mdhPotin.point, parentUI, mdhPotin.relativePoint, mdhPotin.offx, mdhPotin.offy)
	font:SetText(text)
...
---创建文本框
----------------------------------------------------------------------------------------------------------------------
function muc.createTextBox(txtBoxName, parentUI, width, height, mdhPotin, defaultTxt)
	-- 创建一个文本框
	local textBox = CreateFrame("EditBox", txtBoxName, parentUI, "InputBoxTemplate")
	textBox:SetSize(width, height)     -- 设置宽度和高度
	textBox:SetPoint(mdhPotin.point, parentUI, mdhPotin.relativePoint, mdhPotin.offx, mdhPotin.offy)
...
---创建按钮
----------------------------------------------------------------------------------------------------------------------
function muc.createButton(btnName, btnText, parentUI, btnWidth, btnHeight, mdhPotin, btnClickHandler)
	local button = CreateFrame("Button", btnName, parentUI, "UIPanelButtonTemplate")
	button:SetText(btnText)
	button:SetWidth(btnWidth)
	button:SetHeight(btnHeight)
	button:SetPoint(mdhPotin.point, parentUI, mdhPotin.relativePoint, mdhPotin.offx, mdhPotin.offy) -- 将文本框置于屏幕中央
...
function muc.createCheckboxBy3Column(checkBoxName, parentUI, clickX, text, tip, defaultCheck, clickHandler)
	local x, y
	if clickX == 1 then
		x = 16
	...
	local check = CreateFrame("CheckButton", checkBoxName, parentUI, "InterfaceOptionsCheckButtonTemplate")
...
---创建checkBox
----------------------------------------------------------------------------------------------------------------------
function muc.createCheckbox(checkBoxName, parentUI, mdhPotin, text, tip, defaultCheck, clickHandler)
	if text == "" then return end
	local check = CreateFrame("CheckButton", checkBoxName, parentUI, "InterfaceOptionsCheckButtonTemplate")
	check:SetPoint(mdhPotin.point, parentUI, mdhPotin.relativePoint, mdhPotin.offx, mdhPotin.offy)
	check:SetChecked(defaultCheck)
	check.text:SetText(text)
	-- 鼠标移入显示 tip
	check:SetScript("OnEnter", function(self)
...
function muc.createTabPanls(
	tabButtonsFramePoint,
	tabButtonsFrameSize,
	tabContentFramePoint,
	tabContentFrameSize,
	parentUI,
	tabButtons)
	-- 创建 Tab 按钮容器
	local tabButtonsFrame = CreateFrame("Frame", nil, parentUI)
	tabButtonsFrame:SetSize(tabButtonsFrameSize.width, tabButtonsFrameSize.height)
	tabButtonsFrame:SetPoint(
		tabButtonsFramePoint.point,
		parentUI,
	...
	tabContentFrame:SetSize(tabContentFrameSize.width, tabContentFrameSize.height)
	tabContentFrame:SetPoint(
		tabContentFramePoint.point,
		parentUI,
	...
	local function CreateTab(index, tabButton)
		-- Tab 按钮 local
		local currentTabButton = CreateFrame("Button", nil, tabButtonsFrame, "UIPanelButtonTemplate")
