...
mdhelper.mdMaps = {
    ["1669"] = { name = "塞兹迷雾的仙林", enable = true },
    ["g410"] = { name = "通灵战潮", enable = true },
    ["293"] = { name = "格瑞姆巴托", enable = true },
    ["1162"] = { name = "围攻伯拉勒斯", enable = true },
    ["2343"] = { name = "千丝之城", enable = true },
    ["2357"] = { name = "回响之城", enable = true },
    ["2341"] = { name = "矶石宝库", enable = true },
    ["2359"] = { name = "破晨号", enable = true },
...
local function InitializeDB(defaultTables, globalDb)
    ...
    for key, value in pairs(defaultTables) do
        if type(value) == "table" then
            if not globalDb[key] then globalDb[key] = {} end
            InitializeDB(value, globalDb[key])
        else
            if globalDb[key] == nil then globalDb[key] = value end
        end
    end
end
